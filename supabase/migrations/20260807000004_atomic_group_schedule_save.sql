-- Save group attributes and weekly rules in one transaction so a conflict cannot
-- leave the group pointing at a teacher or room that no longer matches its rules.
...
begin
  if jsonb_typeof(coalesce(p_group, '{}'::jsonb)) <> 'object' then
    raise exception 'group_payload_invalid';
  end if;
  ...
  if group_room_id is not null and not exists (
    select 1 from public.rooms
    where id = group_room_id and organization_id = p_organization_id
      and (group_branch_id is null or branch_id = group_branch_id)
  ) then raise exception 'room_not_found_or_wrong_branch'; end if;
  if group_teacher_id is not null and not exists (
    select 1 from public.org_memberships
    where organization_id = p_organization_id and user_id = group_teacher_id and role = 'teacher' and is_active = true
  ) then raise exception 'teacher_not_found'; end if;

  if p_group_id is null then
    insert into public.groups (
      organization_id, title, course_id, branch_id, room_id, teacher_id, status,
      age_from, age_to, capacity, starts_on, ends_on, price_monthly, show_on_site, sort_order
    ) values (
      p_organization_id, group_title, group_course_id, group_branch_id, group_room_id, group_teacher_id, group_status_value,
      nullif(p_group->>'age_from', '')::int,
      nullif(p_group->>'age_to', '')::int,
      coalesce(nullif(p_group->>'capacity', '')::int, 8),
      nullif(p_group->>'starts_on', '')::date,
      nullif(p_group->>'ends_on', '')::date,
      nullif(p_group->>'price_monthly', '')::numeric,
      coalesce((p_group->>'show_on_site')::boolean, true),
      coalesce(nullif(p_group->>'sort_order', '')::int, 100)
