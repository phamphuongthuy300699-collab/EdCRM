...
- **Unrestricted SECURITY DEFINER execution.** Audit found `convert_lead_to_student` and other definer functions inheriting executable grants. Migration `20260811000003_security_grants_hardening.sql` revokes PUBLIC/anon/authenticated, grants service role, and selectively restores only policy helper functions. Covered by 13 pgTAP assertions.
- **Cross-organization Auth identity takeover.** Staff creation previously reused an already registered global email, after which a local administrator could reset that shared user's password. Existing Auth identities are now rejected instead of attached; new staff receives server-only organization ownership metadata. Global email/profile/password mutations require both that marker and a live single-organization check across staff, guardian and student link tables. The migration backfills only identities whose complete organization set is unambiguous, while shared or unmarked identities must use safe account recovery.
