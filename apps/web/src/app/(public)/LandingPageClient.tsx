...
function mediaItems(values: any[] | undefined) {
  return Array.isArray(values) ? values.filter((item) => mediaPath(item) && (typeof item !== "object" || item?.isActive !== false)) : [];
}
    ...
    <div className="site-landing" style={{ fontFamily: "var(--font-inter), sans-serif", color: "var(--color-text)" }}>
      ...
      <section id="projects" style={{ padding: "100px 0", borderBottom: "1px solid var(--color-border)" }}>
        <div className="container">
          ...
          <ImageCollectionGrid images={projectsToRender} layout={studentProjectsLayout} renderItem={(project, index) => <PublicStudentProjectCard key={String(project.id || project.path)} item={project} layout={studentProjectsLayout} index={index} />} />
        </div>
      </section>
      )}
      ...
      <section style={{ padding: "100px 0", borderBottom: "1px solid var(--color-border)" }}>
        <div className="container">
          ...
          <ImageCollectionGrid images={stepsToRender} layout={lessonProcessLayout} renderItem={(step, index) => <PublicLessonStepCard key={String(step.id || step.path)} item={step} layout={lessonProcessLayout} index={index} />} />
        </div>
      </section>
      )}
