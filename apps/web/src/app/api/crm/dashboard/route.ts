...
export async function GET() {
  ...
  const [newLeadsCountResult, newLeadsTodayResult, recentLeadsResult, overdueCountResult, overdueAmountResult, recentInvoicesResult, studentsResult, groupsResult, enrollmentsResult, sessionsResult, debtResult, payrollResult] = await Promise.all([
  ...
  const error = [newLeadsCountResult, newLeadsTodayResult, recentLeadsResult, overdueCountResult, overdueAmountResult, recentInvoicesResult, studentsResult, groupsResult, enrollmentsResult, sessionsResult, debtResult, payrollResult].find((result) => result.error)?.error;
  ...
  return NextResponse.json({
    ...
    stats: {
      ...
      todayCompleted: todaySessions.filter((session: any) => session.status === "completed").length,
      todayRemaining: todaySessions.filter((session: any) => session.status === "planned" || session.status === "live").length,
