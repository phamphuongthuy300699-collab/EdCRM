...
export async function GET() {
  ...
  const [leadsResult, invoicesResult, studentsResult, groupsResult, enrollmentsResult, sessionsResult] = await Promise.all([
    admin.from("leads").select("id, first_name, phone, child_name, status, created_at, courses(title)").eq("organization_id", access.organizationId).order("created_at", { ascending: false }).limit(20),
    admin.from("invoices").select("id, title, amount, status, due_date, students(full_name, student_guardians(guardians(full_name)))").eq("organization_id", access.organizationId).eq("status", "overdue").order("due_date", { ascending: true }).limit(20),
    ...
    admin.from("enrollments").select("id, group_id").eq("organization_id", access.organizationId).eq("status", "active"),
  ...
  const error = [leadsResult, invoicesResult, studentsResult, groupsResult, enrollmentsResult, sessionsResult].find((result) => result.error)?.error;
  ...
  const overdue = invoicesResult.data || [];
  ...
  return NextResponse.json({
    ...
    stats: {
      ...
      newLeadsToday: newLeads.filter((lead: any) => new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Moscow" }).format(new Date(lead.created_at)) === today).length,
      overdueAmount: overdue.reduce((sum: number, invoice: any) => sum + Number(invoice.amount || 0), 0),
