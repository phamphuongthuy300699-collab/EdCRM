...
export default function CrmStudentsPage() {
  ...
  const [newLinkedGuardian,setNewLinkedGuardian]=useState({fullName:"",phone:"",email:""});
  ...
  const linkGuardian=async()=>{if(!selectedStudent||!guardianToLink)return;const response=await fetch("/api/crm/client-relations",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({mode:"link",studentId:selectedStudent.id,guardianId:guardianToLink,relation:"Родитель",isPrimary:!selectedStudent.guardianId,isBillingContact:!selectedStudent.guardianId})});const payload=await response.json();setStudentOperationMessage(response.ok?"Родитель привязан":payload.error||"Не удалось привязать родителя");if(response.ok){setGuardianToLink("");setReloadKey(value=>value+1)}};
  const createAndLinkGuardian=async()=>{if(!selectedStudent||!newLinkedGuardian.fullName.trim())return;const response=await fetch("/api/crm/client-relations",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({mode:"createGuardian",studentId:selectedStudent.id,guardian:newLinkedGuardian,relation:"Родитель",isPrimary:!selectedStudent.guardianId,isBillingContact:!selectedStudent.guardianId})});const payload=await response.json();setStudentOperationMessage(response.ok?"Родитель создан и привязан":payload.error||"Не удалось создать родителя");if(response.ok){setNewLinkedGuardian({fullName:"",phone:"",email:""});setReloadKey(value=>value+1)}};
      ...
      {selectedStudent && (
        <CrmDialog title={<span style={{ display: "flex", alignItems: "center", gap: 10 }}>{selectedStudent.name} {getStatusBadge(selectedStudent.status)}</span>} description={<>Статус: <strong>{selectedStudent.status === "active" ? "Активен" : selectedStudent.status === "paused" ? "Приостановлен" : "Архив"}</strong> · Группа: <strong>{selectedStudent.group || "Без группы"}</strong></>} onClose={closeDrawer} width={520} variant="drawer">
          {selectedStudent.status !== "archived" ? <section className="card-crm" style={{ padding: 14, marginBottom: 16, display: "grid", gap: 10, background: "white" }}>
            ...
            <Button type="button" variant="primary-crm" disabled={savingStudentOperation || !editStudentName.trim()} onClick={saveStudentDetails}>Сохранить данные</Button>
