...
export function AttendanceRoster({ rows, disabled = false, onChange, onSave, onComplete, saving = false, completing = false, sessionStatus, message }: {
  ...
  const canComplete = sessionStatus === "live" && allAttendanceMarked(rows) && !saving && !completing;
  ...
  const statusButton = (row: AttendanceRosterRow, status: AttendanceStatus, label: string) => {
    const selected = row.status === status || (status === "absent_unexcused" && (row.status === "absent_excused" || row.status === "absent_unexcused"));
    return <button type="button" disabled={disabled} aria-pressed={selected} onClick={() => update(row.studentId, { status })} style={{ minHeight: 44, padding: "8px 12px", borderRadius: 9, border: selected ? "2px solid var(--color-primary)" : "1px solid var(--color-border)", background: selected ? "var(--color-primary-soft)" : "white", color: selected ? "var(--color-primary-dark)" : "var(--color-text)", fontWeight: 750, cursor: disabled ? "default" : "pointer" }}>{label}</button>;
  };

  if (!rows.length) return <p style={{ color: "var(--color-text-muted)", padding: "24px 0", textAlign: "center" }}>В занятии пока нет учеников.</p>;
  ...
  return (
    <div className="attendance-roster" style={{ display: "grid", gap: 12, minWidth: 0 }}>
      ...
      {rows.map((row) => {
        ...
        return (
          <article key={row.studentId} style={{ minWidth: 0, border: "1px solid var(--color-border)", borderRadius: 12, padding: 14, background: absent ? "var(--color-danger-soft)" : "white", display: "grid", gap: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" }}><strong style={{ overflowWrap: "anywhere" }}>{row.studentName}</strong>{row.isMakeup && <span className="badge badge-amber">Отработка</span>}</div>
            <div className="attendance-primary-actions" style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 7 }}>
              ...
              {statusButton(row, "absent_unexcused", "Нет")}
            </div>
            {absent && <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 7 }}>{statusButton(row, "absent_excused", "Уважительно")}{statusButton(row, "absent_unexcused", "Без причины")}</div>}
      ...
      {(onSave || onComplete) && <div className="attendance-sticky-footer" style={{ position: "sticky", bottom: 0, zIndex: 10, margin: "4px -16px -16px", padding: "12px 16px max(12px, env(safe-area-inset-bottom))", borderTop: "1px solid var(--color-border)", background: "rgba(255,255,255,.96)", backdropFilter: "blur(8px)", display: "grid", gridTemplateColumns: "1fr auto auto", gap: 8, alignItems: "center" }}><strong style={{ fontSize: 12 }}>Отмечено {marked} из {rows.length}</strong>{onSave && <button type="button" disabled={saving || completing || disabled} onClick={() => void onSave()} style={{ minHeight: 44, padding: "8px 14px", border: "1px solid var(--color-primary)", borderRadius: 9, background: "white", color: "var(--color-primary)", fontWeight: 750 }}>Сохранить</button>}{onComplete && <button type="button" disabled={!canComplete} onClick={() => void onComplete()} style={{ minHeight: 44, padding: "8px 14px", border: 0, borderRadius: 9, background: canComplete ? "var(--color-primary)" : "var(--color-border)", color: canComplete ? "white" : "var(--color-text-muted)", fontWeight: 750 }}>{completing ? "Завершаем…" : "Завершить занятие"}</button>}</div>}
      {message && <div role="status" style={{ color: "var(--color-success)", fontSize: 13, fontWeight: 750 }}>{message}</div>}
      <style jsx>{`@media (max-width: 520px) { .attendance-sticky-footer { grid-template-columns: 1fr 1fr !important; } .attendance-sticky-footer strong { grid-column: 1 / -1; } }`}</style>
    </div>
  );
}
